  ...
  <div class="consumables-page">
      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-button type="primary" @click="handleAdd"><el-icon><Plus /></el-icon> 新增耗材</el-button>
          </div>
        </div>
      </template>

      <el-table :data="tableData" v-loading="loading" stripe border>
        <el-table-column prop="serialNumber" label="产品编码" min-width="120" />
        <el-table-column prop="name" label="耗材名称" min-width="140" />
        <el-table-column prop="category" label="类别" width="120" />
        <el-table-column prop="unit" label="单位" width="70" />
        <el-table-column prop="currentStock" label="库存数量" width="90">
          <template #default="{ row }">
            <span :class="{ 'low-stock': row.currentStock <= row.minStockLevel }">{{ row.currentStock }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="adjustVisible" title="库存调整" width="420px">
      <el-form :model="adjustForm" label-width="80px">
        ...
        <el-form-item label="备注">
          <el-input v-model="adjustForm.remarks" placeholder="可选" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import {
  getConsumablePage, saveConsumable, updateConsumable, deleteConsumable, adjustConsumableStock,
  type ConsumableVO, type ConsumableSaveDTO,
} from '@/api/consumable'
...
const defaultForm: ConsumableSaveDTO = {
  name: '', category: '', unit: '', minStockLevel: 0, maxStockLevel: 0,
  unitCost: 0, supplier: '', storageLocation: '', remarks: '',
}
...
const fetchData = async () => {
  ...
  try {
    const res = await getConsumablePage({
      ...
      name: searchForm.name || undefined, category: searchForm.category || undefined,
    })
...
const handleSave = async () => {
  ...
  try {
    if (isEdit.value && editingId.value) { await updateConsumable(editingId.value, form); ElMessage.success('更新成功') }
    else { await saveConsumable(form); ElMessage.success('新增成功') }
...
const handleDelete = async (row: ConsumableVO) => {
  try {
    ...
    await deleteConsumable(row.id); ElMessage.success('删除成功'); fetchData()
  } catch {}
}
...
.consumables-page { display: flex; flex-direction: column; gap: 12px; padding: 10px; }
.card-header { display: flex; justify-content: space-between; align-items: center; .header-actions { display: flex; gap: 8px; } }
