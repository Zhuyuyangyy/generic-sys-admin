<template>
  <div class="outbound-page">
    <el-row :gutter="16">
      <el-col :span="8">
        <el-card>
          <template #header><span>耗材出库</span></template>
          <el-form :model="form" :rules="formRules" ref="formRef" label-width="80px">
            <el-form-item label="耗材" prop="consumableId">
              <el-select v-model="form.consumableId" placeholder="请选择耗材" style="width: 100%" filterable>
                <el-option v-for="item in consumableList" :key="item.id" :label="`${item.name} (库存: ${item.currentStock})`" :value="item.id" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item>
              <el-button type="warning" :loading="submitLoading" @click="handleSubmit">确认出库</el-button>
            </el-form-item>
          </el-form>
        </el-card>
      </el-col>
      <el-col :span="16">
        <el-card>
          <template #header><span>出库历史</span></template>
          ...
          <el-empty v-if="historyList.length === 0 && !historyLoading" description="暂无出库记录" />
        </el-card>
      </el-col>
    </el-row>
  </div>
...
import { getConsumablePage, outboundConsumable, type ConsumableVO } from '@/api/consumable'
import request from '@/utils/request'

const consumableList = ref<ConsumableVO[]>([])
const submitLoading = ref(false)
const formRef = ref()
const form = reactive({ consumableId: undefined as number | undefined, quantity: 1, reason: '', referenceNo: '', remarks: '' })
const formRules = {
...
const fetchHistory = async () => {
  ...
  try {
    const res: any = await request.get('/consumables/outbound/history')
...
const handleSubmit = async () => {
  ...
  try {
    await outboundConsumable(form.consumableId!, form.quantity, form.referenceNo || undefined, form.remarks || undefined)
    ElMessage.success('出库成功')
    form.consumableId = undefined; form.quantity = 1; form.reason = ''; form.referenceNo = ''; form.remarks = ''
    fetchHistory()
