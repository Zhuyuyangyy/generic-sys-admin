...
-- Encoding: UTF8MB4
--
-- 用法：mysql -u root -p <database_name> < sql/v1.0__init.sql
-- 本文件【不能】包含 CREATE DATABASE / USE：那会把表建到硬编码的
-- generic_sys_admin，而不是调用方指定的目标库，导致后续 migration
-- （v1.2 等）找不到表而中断。建库由调用方负责，例如：
--   CREATE DATABASE erms CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
-- =============================================
...
CREATE TABLE `sys_menu` (
    ...
    `permission`   VARCHAR(128)       DEFAULT NULL            COMMENT 'Permission identifier, e.g. system:user:list',
    `menu_type`    TINYINT UNSIGNED    NOT NULL DEFAULT 2      COMMENT 'Menu type: 1=directory, 2=menu, 3=button',
...
VALUES
    (0, 'Dashboard', '/dashboard', 'dashboard/index', 'el-icon-odometer', 'dashboard:view', 2, 1, 1, 1, 1, NOW()),
    (0, 'User Management', '/users', 'system/user/index', 'el-icon-user', 'system:user:list', 2, 2, 1, 1, 1, NOW()),
    (0, 'Role Management', '/roles', 'system/role/index', 'el-icon-postcard', 'system:role:list', 2, 3, 1, 1, 1, NOW()),
    (0, 'Menu Management', '/menus', 'system/menu/index', 'el-icon-menu', 'system:menu:list', 2, 4, 1, 1, 1, NOW()),
    (0, 'Equipment', '/equipment', 'equipment/index', 'el-icon-set-up', 'equipment:list', 2, 5, 1, 1, 1, NOW()),
    (0, 'Consumables', '/consumables', 'consumable/index', 'el-icon-goods', 'consumable:list', 2, 6, 1, 1, 1, NOW()),
    (0, 'Inventory', '/inventory', 'inventory/index', 'el-icon-box', 'inventory:list', 2, 7, 1, 1, 1, NOW()),
    (0, 'Operation Logs', '/logs', 'system/log/index', 'el-icon-document', 'system:log:list', 2, 8, 1, 1, 1, NOW());
