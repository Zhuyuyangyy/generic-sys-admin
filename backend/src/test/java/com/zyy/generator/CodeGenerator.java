...
import com.baomidou.mybatisplus.generator.config.OutputDir;
            ...
            .strategyConfig(builder -> {
                builder
                    ...
                    .entityBuilder()
                        ...
                        /**
                         * 添加父类公共字段（这些字段在 BaseEntity 已经声明了，这里不需要重复）
                         * 设置为不要这些字段，因为 BaseEntity 已经有了
                         */
                        .excludeColumn("id")
                        .excludeColumn("create_time")
                        .excludeColumn("update_time")
                        .excludeColumn("is_deleted")
                        ...
                        /**
                         * 添加自定义字段（如业务需要的特殊字段）
                         * 这里的字段会在生成的 Entity 中追加
                         */
                        .addColumnFields(
                            new Column("tenant_id").comment("租户ID（多租户隔离用）").go()
                        )
                    ...
                    .serviceBuilder()
                        ...
                        /**
                         * ⚠️ superServiceClass / superServiceImplClass
                         * 继承基础 Service，获得内置的 CRUD 方法
                         * 不需要自己写 save/update/remove 方法！
                         */
                        .superServiceClass(null)         // 使用 MyBatis-Plus 内置 IService
                        .superServiceImplClass(null)     // 使用 MyBatis-Plus 内置 ServiceImpl
                    ...
                    .controllerBuilder()
                        ...
                        /**
                         * ⚠️ naming / tolineName：同上，URL路径映射
                         * bookController → /book → 对应表名
                         */
                        .naming(NamingStrategy.underline_to_camel)
                        .columnNaming(NamingStrategy.underline_to_camel)
                        ...
                        /**
                         * ⚠️ mapping：URL路径映射
                         * 比如 BookController 的 @RequestMapping("/book")
                         * 方法：GET /book/page, POST /book, PUT /book/{id}, DELETE /book/{id}
                         * RESTful 风格，标准写法
                         */
                        .mapping("/" + naming(tableName).toLowerCase())
                ;
            })
